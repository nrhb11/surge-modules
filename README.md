...
- YouTube Music 网页版：只清理接口中的首页、搜索、队列和播放器广告，不修改 Music HTML 页面。
...
未加入 `ctier=L` 和 `oad` 的 `googlevideo.com` 重写。实际抓包显示普通预览与正常视频请求也可能带 `ctier=L`；粗暴重写会造成卡顿、黑屏或清晰度异常。也未加入会把加密初始化数据转发到第三方 Worker 的脚本。
